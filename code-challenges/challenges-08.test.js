...
const typeNum = (arr) => {
  let array = arr.filter((n)=> !isNaN(n));
  return array;
};
...
const filterStringsWithVowels = (arr) => {
  let array = arr.filter(item => item.match(/[aeiou]/));
  return array;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let array = arr.filter((n)=> !forbiddenValues.includes(n));
