...
Write a function named transformToLis that, given an object, returns an array of the key value pairs
as html list items.
...
function transformToLis(obj){
  ...
  for (let i = 0; i < Object.keys(obj).length; i++) {
    array.push(`<li>${(Object.keys(obj)[i]+':'+' '+Object.values(obj)[i])}</li>`);
  }
  return array;
}
...
const reversedString = (str) => {
  let array = str.split('');
  let revString = array.reduce((acc,val)=>{
    acc = val + acc ;
    return acc;
  },'');
  return revString;
