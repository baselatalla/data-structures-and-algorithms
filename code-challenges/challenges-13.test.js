...
const longestString = (arr) => {
  let x = [];
  arr.forEach(n => x.push(n));
  x.sort((a, b) => {
    if (a.length > b.length){return 1;
    } else if (a.length < b.length){
      return -1;} else {return 0;}
  });
  return arr.indexOf(`${x[x.length-1]}`);

 
};
// const longestString = (arr) => {
//   var longest = arr.reduce((x, y)=>{
...
const findHappiness = (arr) => {
  let array = arr.filter(item => item.includes(':)'));
  return array;
};
...
array of phone number strings in (XXX) XXX-XXXX format and returns an 
array with the phone number strings in XXXXXXXXXX format.
